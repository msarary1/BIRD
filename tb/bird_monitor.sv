    ...
    forever begin
      ...
      if (!vif.mon_cb.rst_n) begin
        ...
        if (bytes_left == 0) begin
          ...
          // Normal packet length = payload_len + 2 CRC bytes.
          // Special case:
          // If PAYLOAD_LEN=0, the cfg is invalid, but the DUT still entered
          // RX after seeing a transfer. To keep the monitor synchronized,
          // collect 3 bytes: one dummy transfer + two CRC bytes.
          if (cur.payload_len == 0) begin
            bytes_left = 3;
          end
          else begin
...
class bird_local_monitor;
  ...
  task run();
    ...
    forever begin
      ...
      // Output is counted only on real valid/ready handshake.
      if (vif.mon_cb.rst_n &&
